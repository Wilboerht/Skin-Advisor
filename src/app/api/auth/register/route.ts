...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!ipLimit.success) {
            return apiError(ErrorCode.RATE_LIMITED, "注册尝试过于频繁，请 15 分钟后再试", 429);
        }
        ...
        if (!result) {
            return apiError(ErrorCode.UPSTREAM_ERROR, "注册失败：上游服务响应异常或签名无效", 502);
        }
        ...
        if (!result.ok || !responseData.success) {
            return NextResponse.json(
                { success: false, error: responseData.error || { code: ErrorCode.VALIDATION_ERROR, message: "注册失败" } },
                { status: result.status || 400 }
            );
        }

        if (!responseData.data?.user) {
            return apiError(ErrorCode.UPSTREAM_ERROR, "注册失败：上游响应格式异常", 502);
        }
