...
export async function POST(req: NextRequest) {
    try {
        ...
        const prompt = `将图片中的人像转为日系美妆插画风格，【核心要求：必须具备极高的人貌相似度，严格保留原照片的五官比例、脸型特征、眼神和发型等所有个人身份细节，仅仅是给照片加上插画滤镜感，绝对不能改变本人的长相特征，要让人能一眼认出就是本人】（性别：${gender}，大约${age}岁，肤色：${skinTone}，发型：${hairStyle}），【构图绝对要求：人物必须严格位于画面的垂直中轴线上，左右两侧背景留白必须完全对称均等】，必须是标准的正面头像视角，人物的头部和身体都必须笔直正对镜头，绝对不能歪头、侧身或歪着身子，仅保留单一主体人物，【背景绝对规定：彻底清除原图背景！必须是极简的、完全平面的均一单色背景（淡米色或纯白色），背景上绝对不能有任何渐变色、阴影、光斑、线条、纹理、图案或残影，整个背景必须完全是一模一样的纯色】，适当轻度美颜并保留本人原本骨相特色、给角色自然微笑表情`;
