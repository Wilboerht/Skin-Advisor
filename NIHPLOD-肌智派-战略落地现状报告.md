...
核心闭环"测肤 → 匹配 → 追踪 + 7×24 AI 顾问"已跑通，数据资产策略（90 天滚动续期 + 冷热分层）已落地。剩余待办仅一项：**P0 三级会员体系（待讨论后实施）**。营销预案大部分为外部宣发工作，系统侧需支撑的 GEO、分享裂变、白皮书数据导出均已就绪。
...
| 2.4 品牌/产品向量数据 | ⛔ 已否决 | 仅 9 个 SKU，规则匹配够用，不做向量库 |
...
| 4.10 白皮书 | 外部宣发 | ✅ 数据导出已实现（后台「白皮书统计数据」CSV，脱敏群体统计），内容撰写与发布为外部工作 |
...
| ⛔ 已否决 | 品牌/产品向量数据（2.4） | 仅 9 个 SKU，规则匹配完全够用，向量检索无收益；知识库全文塞提示词也塞得下。若未来大幅扩品再议 |
| ⛔ 已否决 | 主站应用内 AI 聊天 | 顾问对话统一走微信生态（企微客服通道），主站不重复建设 |
| ✅ 已实现 | 白皮书数据导出（2026-08-18） | 后台「数据报表 → 白皮书统计数据」一键导出 CSV：每条已完成测肤一行的脱敏统计字段（AI 肤质/persona/综合分/四维度分/年龄段/预算/关注问题/省份/测肤方式/分享渠道/冷热层标记），热层冷层通吃，无身份标识、无敏感问卷字段、地域只到省级 |
...
| 归档脱水/统计提取逻辑 | `src/lib/session-archive.ts` |
| 白皮书统计数据导出 | `src/app/api/admin/export/route.ts`（`?type=whitepaper`） |
